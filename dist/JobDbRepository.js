    ...
    async saveJob(job) {
        try {
            ...
            if (id) {
                ...
                // @ts-ignore
                const result = (await this.collection.findOneAndUpdate({ _id: id, name: props.name }, update, {
                    returnDocument: 'after',
                    includeResultMetadata: true
                }));
            ...
            if (props.type === 'single') {
                ...
                log(`findOneAndUpdate(${props.name}) with type "single" ${result.lastErrorObject?.updatedExisting
